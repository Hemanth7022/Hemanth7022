<div align="center">



</div>

---

## 🧑‍💻 About Me

Hey! I'm **Hemanth**, a  AI & ML student I build things at the intersection of **machine learning**, **graph neural networks**, and **real-world problem solving** — from detecting UPI fraud to making cities smarter with data.

I love working on problems that actually matter. Whether it's catching financial fraud before it happens or helping city planners make better decisions, I'm all about using AI where it counts.



---

## 🚀 Current Projects

### 🕵️ GNN-Based Fraud Detection in UPI
> Using **Graph Neural Networks** to model UPI transaction networks and identify fraudulent patterns in real time.

- Built a graph where **nodes = users/merchants** and **edges = transactions**
- Leveraging **PyTorch Geometric** to train GNN models that learn suspicious behavioral patterns
- Tackling class imbalance, dynamic graph updates, and real-world noise in financial data
- Goal: Flag fraudulent transactions **before** they complete

### 🏙️ Urban Traffic Planning AI
> A data-driven system that helps city planners analyze and optimize traffic flow across Bangalore.

- Integrates **geospatial data** with ML models to predict congestion hotspots
- Uses multiple AI agents to score incident probability and suggest routing decisions
- Generates automated explanations for **drivers, engineers, and city regulators**
- Built with Python, Pandas, and real urban dataset pipelines

---

## 🛠️ Tech Stack

### 🤖 Machine Learning & AI
<div align="center">

![Tech](https://skillicons.dev/icons?i=python,pytorch,tensorflow,sklearn&theme=dark)

</div>

| Library | Use |
|---|---|
| **PyTorch & PyTorch Geometric** | GNNs, deep learning models |
| **Scikit-learn** | Classical ML, preprocessing, evaluation |
| **OpenCV** | Computer vision, real-time detection |
| **Pandas & NumPy** | Data wrangling & analysis |
| **Matplotlib & Seaborn** | Data visualization |

### 🌐 Web Development
<div align="center">

![Web](https://skillicons.dev/icons?i=html,css,javascript,mysql&theme=dark)

</div>

### 🧰 Tools & Platforms
<div align="center">

![Tools](https://skillicons.dev/icons?i=git,github,vscode,jupyter&theme=dark)

</div>

---

## 📬 Connect with Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-Hemanth7022-181717?style=for-the-badge&logo=github)](https://github.com/Hemanth7022)
[![Email](https://img.shields.io/badge/Email-hemanth2005hk@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hemanth2005hk@gmail.com)

</div>

---

<div align="center">

*"Data is not just numbers — it's the story of the world waiting to be told."*

![footer](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,24&height=120&section=footer)

</div>
